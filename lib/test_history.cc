...
static const cli_cmd_list_t mock_cmd_list = {NULL, 0, mock_cmds, 3};
...
TEST_F(CliHistoryTest, PushAndList) {
  // Manually push some commands to history since we want to test the history
  // command first Note: in actual use, cli_mainloop calls cli_history_push
  ...
  for (const auto &line : output_lines) {
    if (line.find("help") != std::string::npos)
      found_help = true;
    if (line.find("echo on") != std::string::npos)
      found_echo = true;
  }
...
TEST_F(CliHistoryTest, NoSaveOnFailure) {
  ...
  for (const auto &line : output_lines) {
    if (line.find("unknown_cmd") != std::string::npos)
      found_unknown = true;
  }
